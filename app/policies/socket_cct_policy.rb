class SocketCctPolicy < ApplicationPolicy
  # Returns the socket_cct record
  def socket_cct
    record
  end

  # Required functional role to access socket circuits
  ...
  class Scope < ApplicationPolicy::Scope
    def resolve
      if user&.is_app_owner? || user&.is_admin?
        ...
        # Get all socket circuits that are tagged with the current project
  ...
  def index?
    # Any authenticated user can view the socket circuits index
    # (actual socket circuits are filtered by scope based on project membership)
    user.present?
  end
  ...
  def show?
    ...
    # Admins and app owners can view any socket circuit
    return true if user.is_app_owner? || user.is_admin?
    
    # Check if record is in scope for the current user
    SocketCctPolicy::Scope.new(user_context, SocketCct).resolve.exists?(id: record.id)
  end
  ...
  def create?
    ...
    # Admins and app owners can create any socket circuit
    return true if user.is_app_owner? || user.is_admin?
  ...
  def update?
    ...
    # Admins and app owners can update any socket circuit
    return true if user.is_app_owner? || user.is_admin?
