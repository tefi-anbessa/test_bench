class MotorPolicy < ApplicationPolicy
  # Returns the motor record
  def motor
    record
  end

  # Required functional role to access motors
  ...
  class Scope < ApplicationPolicy::Scope
    def resolve
      if user&.is_app_owner? || user&.is_admin?
        ...
        # Get all motors that are tagged with the current project
  ...
  def index?
    # Any authenticated user can view the motors index
    # (actual motors are filtered by scope based on project membership)
    user.present?
  end
  ...
  def show?
    ...
    # Admins and app owners can view any motor
    return true if user.is_app_owner? || user.is_admin?
    
    # Check if record is in scope for the current user
    MotorPolicy::Scope.new(user_context, Motor).resolve.exists?(id: record.id)
  end
  ...
  def create?
    ...
    # Admins and app owners can create any motor
    return true if user.is_app_owner? || user.is_admin?
  ...
  def update?
    ...
    # Admins and app owners can update any motor
    return true if user.is_app_owner? || user.is_admin?
