...
module Electrical
  class CableTypePolicyTest < ActiveSupport::TestCase
    ...
    def policy(user, project, record = nil)
      ...
      Electrical::CableTypePolicy.new(user_context, record || Electrical::CableType)
    end
    
    # Scope Tests
    test 'scope returns all items for admin users' do
      context = ApplicationPolicy::UserContext.new(@admin, nil)
      scope = Electrical::CableTypePolicy::Scope.new(context, Electrical::CableType).resolve
      assert_equal Electrical::CableType.all, scope
    end
