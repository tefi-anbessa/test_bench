...
class TagablesControllerTest < ActionController::TestCase
  ...
  def test_common_setup_is_valid
    assert tagable_type.in?(Tag.safe_tagable_types)
  ...
  def test_team_member_cannot_access_new_form
    ...
    get :new, params: { tag_id: @unassigned_tag.id, tagable_type: tagable_type }
  ...
  def test_accredited_user_can_create_with_existing_unassigned_tag
    ...
    assert_difference("#{resource_class}.count", 1) do
      post :create, params: create_params.merge(tag_id: @unassigned_tag.id)
    end
  ...
  def test_accredited_user_can_create_new_resource_and_tag
    ...
    assert_difference("Tag.count", 1) do
      assert_difference("#{resource_class}.count", 1) do
        post :create, params: create_params.deep_merge(tag_params).merge(discipline_id: @discipline.id).merge(tagable_type: tagable_type)
      end
    end
    # Find the created resource and tag
    created_tag = Tag.find_by(tag_params[:tag])
  ...
  private
    ...
    def extend_tagable_test
      extension = "#{resource_class.name}TestExtension".safe_constantize
    ...
    def create_params
      raise NotImplementedError, "Extension module must implement create_params"
    end

    def invalid_param
      raise NotImplementedError, "Extension module must implement invalid_param"
    end

    def updated_attribute_value(original_value)
      raise NotImplementedError, "Extension module must implement updated_attribute_value"
    end

    def update_param
      raise NotImplementedError, "Extension module must implement update_param"
    end
