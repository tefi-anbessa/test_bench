...
class CircuitTest < ActiveSupport::TestCase
  ...
  test "should require unique serial number per switchboard" do
    ...
    test_serial = create(:electrical_circuit, switchboard: @switchboard, serial: 35)

    # Try to create another circuit with the same serial on the same switchboard
    duplicate_circuit = build(:electrical_circuit, switchboard: @switchboard, serial: test_serial.serial)
    ...
    other_tag = create(:tag, discipline: @discipline)
  ...
  test "destroy circuit should nullify cable feeder reference" do
    circuit = create(:electrical_circuit, switchboard: @switchboard)
    cable_tag = create(:tag, discipline: @discipline)
  ...
  test "factory should create multiple circuits with unique serial numbers" do
    ...
    circuits = create_list(:electrical_circuit, 3, switchboard: @switchboard)
  ...
  test "factory should enforce unique serial numbers per switchboard" do
    ...
    test_tag = create(:tag, project: @project, discipline: @discipline)
    test_switchboard = create(:electrical_switchboard, tag: test_tag)

    # First, create a circuit with serial 1 on the test switchboard
    create(:electrical_circuit, switchboard: test_switchboard, serial: 1)

    # Try to create another circuit with the same serial on the same switchboard
    duplicate_circuit = build(:electrical_circuit, switchboard: test_switchboard, serial: 1)
    ...
    other_tag = create(:tag, project: @project, discipline: @discipline)
    other_switchboard = create(:electrical_switchboard, tag: other_tag)
    other_circuit = build(:electrical_circuit, switchboard: other_switchboard, serial: 1)
